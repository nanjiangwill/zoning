...
if "all_batches" not in st.session_state:
    if os.path.exists(batched_data_path):
        ...
        st.session_state["all_batches"] = json.loads(open(batched_data_path).read())
    else:
        ...
        st.session_state["all_batches"] = all_batches
...
all_towns = sorted(list(set([i["town_name"] for batch in st.session_state["all_batches"] for i in batch])))
...
if "labelled_data" not in st.session_state:
    st.session_state["labelled_data"] = get_firebase_data(
...
total_num_batches, finished_num_batches, next_batch_info = get_next_unlabeled_batch(
    st.session_state["labelled_data"], st.session_state["all_batches"]
)
        ...
        if entire_search_page_range == []:
            display_info.append(
                f"""
                    <div class="card">
                        <p class="title">
                            {item_i}. <em>{format_eval_term[eval_term]}</em> / {place.district_full_name} ({place.district_short_name})
                        </p>
...
"""
    ...
    return all_showed_pages, all_highlight_info, final_html, item_info

if "current_batch_data" not in st.session_state or not st.session_state["current_batch_data"]:
    all_showed_pages, all_highlight_info, final_html, item_info = process_batch_with_progress(batch)
    st.session_state["current_batch_data"] = {
        "all_showed_pages": all_showed_pages,
        "all_highlight_info": all_highlight_info,
...
if "to_be_highlighted_pages" not in st.session_state or not st.session_state["to_be_highlighted_pages"]:
    st.session_state["to_be_highlighted_pages"] = get_edited_pages(
        st.session_state["current_batch_data"]["all_showed_pages"],
        st.session_state["current_batch_data"]["all_highlight_info"],
...
for k in range(len(st.session_state["to_be_highlighted_pages"]) // 3 + 1):
    for j in range(3):
        i = k * 3 + j
        if i >= len(st.session_state["to_be_highlighted_pages"]):
            continue
        page_img_cols[j].image(
            st.session_state["to_be_highlighted_pages"][i],
...
st.html(st.session_state["current_batch_data"]["final_html"])

cols = st.columns(len(st.session_state["current_batch_data"]["item_info"]))
for item in st.session_state["current_batch_data"]["item_info"]:
    def on_change(idx):
        def _on_change():
            st.session_state["current_batch_data"]["item_info"][idx]["selected"] = not st.session_state["current_batch_data"]["item_info"][idx]["selected"]

        return _on_change
    with cols[item["item_i"] - 1]:
        st.checkbox(str(item["item_i"]), value=item["selected"], on_change=on_change(item["item_i"] - 1))
...
def jump_to_next_batch():
    ...
    if next_batch_info:
        ...
        st.session_state["to_be_highlighted_pages"] = None
...
if next_batch_index < len(st.session_state["all_batches"]):
    next_batch = st.session_state["all_batches"][next_batch_index]
