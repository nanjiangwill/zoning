...
for idx, item in enumerate(st.session_state["current_batch_data"]["item_info"]):
    if item["rationale"]:
        button_text = (
            f'*{item["eval_term"]}* / {item["district_full_name"]} ({item["district_short_name"]})\n\n'
            f'**Value:** {item["value"]}\n\n'
    ...
    if st.button(button_text, key=f"buttonOn{idx}" if item["selected"] else f"buttonOff{idx}"):
        st.session_state["current_batch_data"]["item_info"][idx]["selected"] = not item["selected"]
        st.rerun()
