...
with st.sidebar:
    ...
    st.divider()
    st.subheader("Step 2: Select Data", divider="rainbow")

    def show_fullname_shortname(place, eval_term):
        # jstr = "-".join([i[0].upper() + i[1:] for i in town.split("-")])
...
selected_district = st.session_state["current_district"]
eval_term, district_info = selected_district.split(" - ", 1)
...
# loading info
eval_term = visualized_data["search"][0].eval_term
                ...
                for llm_rect in llm_answer_rects:
                    if any(
                        ...
                        for rect in overlapping_eval_term_rects:
                            to_be_highlighted_eval_term_rects.append([rect, 0.05])
