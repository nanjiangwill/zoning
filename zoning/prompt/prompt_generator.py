...
from zoning.utils import prompt_file, get_thesaurus
from zoning.class_types import SearchResult, PromptResult, Prompt

class PromptGenerator(ABC):
    def __init__(self, prompt_config):
        self.prompt_config = prompt_config
        self.prompt_env = Environment(loader=FileSystemLoader(prompt_config.templates_dir))
        self.prompt_template = self.prompt_env.get_template(prompt_file(prompt_config.method))
        

    def generate_prompt(self, search_result: SearchResult, target: str) -> PromptResult:
        synonyms = ", ".join(
            get_thesaurus(self.prompt_config.thesaurus_file).get(search_result.eval_term, [])
        )
