...
from zoning.class_types import DistrictExtractionConfig, DistrictExtractionResult, PageEmbeddingResult,FormatOCR
from zoning.search.keyword_searcher import KeywordSearcher 
from zoning.utils import prompt_file, post_processing_llm_output, flatten
...
class DistrictExtractor:
    ...
    def page_embedding(
        ...
        emb = self.openai_client.embeddings.create(input=y, model=self.district_extraction_config.embedding_model)
    ...
    def district_extraction(
        ...
        if self.QUERY_EMBEDDING is None:
            query_embedding_response = self.openai_client.embeddings.create(
                    input=self.QUERY,
                    model=self.district_extraction_config.embedding_model
                )
    ...
    def district_extraction_verification(
        ...
        for d in x.districts:
            ...
            district_query = KeywordSearcher.get_district_query(district_full_name, district_short_name, False)
